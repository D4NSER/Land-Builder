...
public partial class MainController : Node3D
{
    ...
    public override void _Ready()
    {
        ...
        _currentTileLabel = GetNode<Label>("CanvasLayer/HUDRoot/TileDeckPanel/DeckVBox/CurrentTileLabel");
        _rotationLabel = GetNode<Label>("CanvasLayer/HUDRoot/TileDeckPanel/DeckVBox/RotationLabel");
    ...
    private void WireUi()
    {
        GetNode<Button>("CanvasLayer/HUDRoot/ActionsRail/RailVBox/DrawButton").Pressed += () => Issue(new DrawTileCommand());
        GetNode<Button>("CanvasLayer/HUDRoot/ActionsRail/RailVBox/SubmitScoreButton").Pressed += () => Issue(new SubmitScoreCommand());
        GetNode<Button>("CanvasLayer/HUDRoot/ActionsRail/RailVBox/SaveButton").Pressed += SaveGame;
        GetNode<Button>("CanvasLayer/HUDRoot/ActionsRail/RailVBox/LoadButton").Pressed += LoadGame;
        GetNode<Button>("CanvasLayer/HUDRoot/ActionsRail/RailVBox/RefreshHighScoreButton").Pressed += RefreshHighScoreLabel;
    ...
    private void ApplyHudStyle()
    {
        ...
        _eventLog.CustomMinimumSize = new Vector2(0, 104);
        _eventLog.AddThemeFontSizeOverride("normal_font_size", 11);
    ...
    private void StartNewSession(ulong seed)
    {
        ...
        _renderedTiles.Clear();
        ClearAllTileVisuals();
        SetStatus($"Started new session with seed {seed}.");
        Render();
    }
    ...
    private void Render()
    {
        ...
        _currentTileLabel.Text = $"Current Tile: {(state.CurrentTile?.ToString() ?? "(none)")}";
        _rotationLabel.Text = $"Rotation: {_rotationQuarterTurns * 90} deg  (Q/E)";
        _unlockedTilesLabel.Text = $"Unlocked: {string.Join(", ", state.UnlockedTiles.OrderBy(t => t))}";
        _lastMessageLabel.Text = $"Last Message: {state.LastMessage}";
        _eventLog.Text = string.Join('\n', _eventSink.RecentLines);
    ...
    private void BuildPlains(Node3D root, int seed, NeighborTypes neighbors, bool preview)
    {
        ...
        AddMicroLumps(root, seed, preview, 3, new Color(0.34f, 0.43f, 0.26f));

        var dirtPatchCount = 1;
        for (var i = 0; i < dirtPatchCount; i++)
        {
            var pos = RandomTopPoint(rng, 0.46f);
            var patch = CreateColoredMesh(
                new BoxMesh { Size = new Vector3(rng.RandfRange(0.20f, 0.34f), 0.02f, rng.RandfRange(0.12f, 0.22f)) },
                pos + new Vector3(0, 0.205f, 0),
                new Color(0.31f, 0.33f, 0.22f),
                preview);
        ...
        {
            var pos = RandomTopPoint(rng, 0.55f);
            for (var q = 0; q < 2; q++)
            {
                var blade = CreateColoredMesh(
                    new PlaneMesh { Size = new Vector2(0.14f, rng.RandfRange(0.20f, 0.32f)) },
                    pos + new Vector3(0, 0.21f + rng.RandfRange(-0.01f, 0.05f), 0),
                    new Color(0.30f, 0.52f, 0.22f),
                    preview);
    ...
    private void BuildWoods(Node3D root, int seed, NeighborTypes neighbors, bool preview)
    {
        ...
        var treeCount = rng.RandiRange(2, 4);
        for (var i = 0; i < treeCount; i++)
        {
            var point = RandomTopPoint(rng, 0.52f);
        ...
        {
            var logPos = RandomTopPoint(rng, 0.45f);
    ...
    private void BuildRiver(Node3D root, int seed, NeighborTypes neighbors, bool preview)
    {
        ...
        BuildConnectedRiverWater(root, seed, neighbors, preview);
    ...
    private void BuildMeadow(Node3D root, int seed, NeighborTypes neighbors, bool preview)
    {
        ...
        AddMicroLumps(root, seed, preview, 2, new Color(0.37f, 0.46f, 0.28f));

        for (var i = 0; i < 2; i++)
        {
            var pos = RandomTopPoint(rng, 0.54f);
            var blade = CreateColoredMesh(
        ...
        for (var i = 0; i < rng.RandiRange(4, 5); i++)
        {
            var pos = RandomTopPoint(rng, 0.58f);
        ...
        for (var i = 0; i < 1; i++)
        {
            var pos = RandomTopPoint(rng, 0.52f);
    ...
    private void BuildVillage(Node3D root, int seed, NeighborTypes neighbors, bool preview)
    {
        ...
        var homeCount = rng.RandiRange(2, 3);
        for (var i = 0; i < homeCount; i++)
        {
            var pos = RandomTopPoint(rng, 0.42f);
    ...
    private void BuildLake(Node3D root, int seed, NeighborTypes neighbors, bool preview)
    {
        ...
        BuildWaterLayers(root, seed, preview, isLake: true, rng.RandfRange(-0.1f, 0.1f), new Vector3(0.54f, 0.14f, 0.54f));

        for (var i = 0; i < 2 + rng.RandiRange(0, 1); i++)
        {
            var rock = CreateColoredMesh(
                new SphereMesh { Radius = 0.05f, Height = 0.06f, RadialSegments = 8, Rings = 5 },
    ...
    private void BuildConnectedRiverWater(Node3D root, int seed, NeighborTypes neighbors, bool preview)
    {
        ...
        {
            BuildWaterLayers(root, seed, preview, isLake: false, 0f, new Vector3(0.38f, 0.135f, 1.18f));
            BuildWaterLayers(root, seed + 1, preview, isLake: false, Mathf.Pi * 0.5f, new Vector3(0.38f, 0.135f, 1.18f));
        }
        else if (horizontal)
        {
            BuildWaterLayers(root, seed, preview, isLake: false, Mathf.Pi * 0.5f, new Vector3(0.46f, 0.135f, 1.18f));
        }
        else
        {
            BuildWaterLayers(root, seed, preview, isLake: false, 0f, new Vector3(0.46f, 0.135f, 1.18f));
        }
        ...
        foreach (var dir in CardinalDirs())
        {
            ...
            var bridge = CreateColoredMesh(
                new BoxMesh { Size = new Vector3(0.34f, 0.016f, 0.44f) },
        ...
        if (connectionCount == 0)
        {
            ...
            BuildWaterLayers(root, seed + 9, preview, isLake: false, yaw, new Vector3(0.42f, 0.135f, 1.08f));
        }
    }
    ...
    private float SampleTerrainOffset(TileType tileType, int visualSeed, Vector2 point)
    {
        ...
        var result = noise * amplitude;
        var radius = point.Length();

        if (tileType == TileType.River)
        {
            var axis = point.X * 0.86f + point.Y * 0.35f;
            var radialFalloff = Mathf.Exp(-(point.LengthSquared()) * 1.8f);
            result -= Mathf.Exp(-(axis * axis) * 6.5f) * (0.085f * radialFalloff + 0.01f);
        }
        else if (tileType == TileType.Lake)
        {
            result -= Mathf.Exp(-(radius * radius) * 5.4f) * 0.12f;
        }
    ...
    private void SetStatus(string message) => _statusLabel.Text = $"Status: {message}";
